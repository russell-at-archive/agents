...
| ----------------------------- | ------------------------------------------ |
| `~/.claude/.credentials.json` | Session data and OAuth credentials (Linux) |
| `~/.claude.json`              | Additional configuration and session state |

On macOS, the auth token is stored in the **system keychain**, not on the filesystem. 
You can export the data from the keychain to filesystem for mounting into a dev 
container. 

Share your auth token with a dev container, you can mount the `~/.claude/` directory and `~/.claude.json` file into the container.
