...
| -------------------------------- | ------------------------------------ |
| `~/.gemini/oauth_creds.json`     | OAuth credentials and session tokens |
| `~/.gemini/google_accounts.json` | Google account configuration         |

Share your auth token with a dev container, you can mount the `~/.gemini/` directory into the container.
...
March 2026 Gemini not great at following directions, especially if it encounters errors as its internal directives favor accomplishing the task over adhering to any specific process.