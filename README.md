...
- [Gemini CLI](https://gemini.google.com/) (`gemini`) — optional
...
```bash
git clone git@github.com:russelltsherman/agents.git ~/.agents
```

### 2. Connect these skills to claude code

Symlink this skills directory to `~/.claude/skills/` so Claude Code discovers them:

```bash
ln -s ~/.agents/skills ~/.claude/skills
```

Both Codex and Gemini will find and load skills from `~/.agents/skills` without additional effort.

### 3. Reference shared directives in your project

In your local project add a pointer to the shared Agents.md file
This is not strictly necessary but if you want to have a core set of directives for all agent types this is the way.
...
- [using-claude-agent-sdk](./skills/using-claude-agent-sdk/SKILL.md): Provides
  expert guidance for building AI agents with the Claude Agent SDK (Python
  `claude_agent_sdk` or TypeScript `@anthropic-ai/claude-agent-sdk`). Use when
  building, designing, debugging, or extending agents with custom tools, hooks,
  subagents, MCP servers, or session management.
...
- [writing-editorconfig](./skills/writing-editorconfig/SKILL.md): Produces
  correct, portable `.editorconfig` files following the EditorConfig
  specification. Use when asked to write, create, fix, review, or improve an
  `.editorconfig` file, editor config, or coding style configuration for
