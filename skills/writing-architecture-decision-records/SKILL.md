...
2. **Next ADR number** — check the existing `docs/adr/` directory
...
1. Check whether any ADR exists in `docs/adr/`.
2. If none exists, the first ADR **must** document the decision to adopt ADRs
   for architectural decisions in the project.
3. Determine the next sequential number and derive the filename:
   `NNNN-short-hyphenated-title.md` stored in `docs/adr/`.
4. Write the ADR section by section — see
   [references/overview.md](references/overview.md) for section-writing rules.
5. Run the quality checklist (below) before presenting the file.
6. Output the complete file, ready to commit.
...
- If there are zero ADRs in the project, ADR `0001` must record the decision to
  use ADRs in the project.
