...
kustomize localize github.com/org/repo//deploy/overlays/prod ./vendor/prod
...
Use `build` for rendering, `edit` for mechanical in-place mutations,
`create` for bootstrapping a new root, `localize` to vendor a remote
kustomization before editing it, and `cfg`/`fn` only when the task is really a
KRM configuration-function workflow. `localize`, `cfg`, and `fn` are still
alpha-facing surfaces; use them deliberately.
